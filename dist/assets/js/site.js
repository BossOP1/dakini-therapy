...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (navBtn && navPanel) {
    const header = document.querySelector('[data-header]')
    const navLabel = navBtn.querySelector('[data-nav-label]')
    ...
    const toggleNav = (open) => {
      ...
      if (header) header.toggleAttribute('data-nav-open', open)

      clearTimeout(unmount)

      if (open) {
        // Measure the scrollbar before hiding it, so fixed children can be
        // padded by exactly the width the page is about to lose.
        root.style.setProperty('--sbw', (window.innerWidth - root.clientWidth) + 'px')
        root.classList.add('nav-locked')
        lastFocus = document.activeElement
        navPanel.hidden = false
        // Two frames: one to apply the closed state, one to leave it.
        ...
        // Keep it mounted until the fade finishes, then take it out of the
        // tab order. isReduced skips the wait entirely.
        unmount = setTimeout(() => { navPanel.hidden = true }, isReduced ? 0 : 320)
    ...
    window.addEventListener('keydown', (e) => {
      ...
      // Focus stays inside the overlay while it covers the page. The toggle is
      // part of the loop so the menu can always be closed from the keyboard.
      const items = [navBtn].concat(focusables())
  ...
  // ── 2b. Form success on return from Netlify ────────────────
  // The static build has no PHP, so the "thank you" state arrives as ?sent=1
  // rather than as a POST response.
  ...
  // ── 6. Recommended books filter ───────────────────────────
  const bookPills = document.querySelectorAll('[data-book-filter]')
  if (bookPills.length) {
    const sections = document.querySelectorAll('[data-book-section]')
    const ON  = ['border-ink', 'bg-ink', 'text-citron']
    const OFF = ['border-sand-300', 'bg-paper-lighter', 'text-sand-700']

    bookPills.forEach(pill => {
      pill.addEventListener('click', () => {
        const want = pill.dataset.bookFilter
        bookPills.forEach(p => {
          const on = p === pill
          p.classList.toggle(ON[0], on);  p.classList.toggle(ON[1], on);  p.classList.toggle(ON[2], on)
          p.classList.toggle(OFF[0], !on); p.classList.toggle(OFF[1], !on); p.classList.toggle(OFF[2], !on)
          p.setAttribute('aria-pressed', String(on))
        })
        sections.forEach(sec => {
          sec.hidden = want !== 'all' && sec.dataset.bookSection !== want
        })
      })
    })
  }
  ...
  function initTestimonialToggles() {
    document.querySelectorAll('[data-clamp]').forEach(quote => {
      ...
      const isExpanded = quote.classList.contains('line-clamp-none')
      if (!isExpanded) {
        const isClipped = quote.scrollHeight > quote.clientHeight + 2
        toggle.hidden = !isClipped
      } else {
        toggle.hidden = false
      }
      ...
      if (!toggle.dataset.clampBound) {
        ...
        toggle.addEventListener('click', () => {
          const nowExpanded = quote.classList.toggle('line-clamp-none')
          toggle.textContent = nowExpanded ? 'Read less' : 'Read more'
        })
      }
    })
  }
  ...
  window.addEventListener('resize', () => {
    ...
    clampResizeTimer = setTimeout(initTestimonialToggles, 100)
  })
